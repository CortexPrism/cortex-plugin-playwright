...
function validateUrl(url: unknown): string | null {
  ...
  if (!url.startsWith("https://") && !url.startsWith("http://")) return "URL must start with https:// or http://";
  return null;
}

function validateSelector(sel: unknown): string | null {
  if (!sel || typeof sel !== "string" || sel.trim().length === 0) return "Selector must be a non-empty string";
  return null;
}
...
const browserNavigate: Tool = {
  definition: {
    ...
    description: "Navigate the browser to a URL. Returns the page title and text content after load.",
    params: [
      { name: "url", type: "string", description: "The URL to navigate to (must be HTTPS)", required: true },
      { name: "waitUntil", type: "string", description: "When to consider navigation complete", required: false,
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (urlErr) return { toolName, success: false, output: "", error: urlErr, durationMs: Date.now() - start };
...
const browserScreenshot: Tool = {
  definition: {
    ...
    params: [
      { name: "url", type: "string", description: "URL to screenshot (if not provided, uses last navigated URL)", required: false },
      { name: "fullPage", type: "boolean", description: "Whether to capture the full scrollable page (default: true)", required: false },
    ],
    ...
    try {
      ...
      return {
        ...
        output: JSON.stringify({
          url, title,
          note: "Full browser screenshot requires Playwright CLI installation. Install via: npx playwright install chromium",
...
const browserClick: Tool = {
  definition: {
    name: "browser_click",
    description: "Click on an element matching a CSS selector. Requires Playwright to be installed for full interaction.",
    params: [
      { name: "selector", type: "string", description: "CSS selector for the element to click", required: true },
      { name: "timeout", type: "number", description: "Max time to wait for the element in ms (default: 5000)", required: false },
    ],
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (selErr) return { toolName, success: false, output: "", error: selErr, durationMs: Date.now() - start };

      return {
        toolName, success: true,
        output: JSON.stringify({
          clicked: args.selector,
          note: "Interactive browser operations (click, type, evaluate) require the Playwright runtime. Install via: npx playwright install && npx playwright install-deps",
        }),
        durationMs: Date.now() - start,
      };
    } catch (error) {
    ...
    try {
      ...
      return {
        ...
        output: JSON.stringify({
          ...
          note: "Interactive browser operations require Playwright runtime. See browser_click for install instructions.",
        }),
        durationMs: Date.now() - start,
      };
    } catch (error) {
...
const browserExtract: Tool = {
  definition: {
    ...
    params: [
      { name: "url", type: "string", description: "URL to extract data from (uses HTTPS fetch when Playwright is unavailable)", required: true },
      { name: "selector", type: "string", description: "CSS selector for the element(s) to extract", required: true },
      { name: "attribute", type: "string", description: "HTML attribute to extract (e.g., 'href', 'src'). Omit for text content.", required: false },
      { name: "multiple", type: "boolean", description: "Extract from all matching elements (default: false)", required: false },
    ],
    capabilities: ["network:fetch"],
  },
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      if (!args.url || typeof args.url !== "string") {
        return { toolName, success: false, output: "", error: "url must be a non-empty string", durationMs: Date.now() - start };
      }
      const selErr = validateSelector(args.selector);
      if (selErr) return { toolName, success: false, output: "", error: selErr, durationMs: Date.now() - start };
      ...
      if (attribute) {
        const regex = new RegExp(`<[^>]*${selector.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}[^>]*${attribute}\\s*=\\s*["']([^"']*)["'][^>]*>`, "gi");
...
const browserEvaluate: Tool = {
  definition: {
    ...
    description: "Execute JavaScript in the browser context. Requires Playwright for full JS execution.",
    params: [
      { name: "script", type: "string", description: "JavaScript code to execute in the browser page context", required: true },
    ],
    capabilities: ["shell:run"],
  },
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      if (!args.script || typeof args.script !== "string") {
        return { toolName, success: false, output: "", error: "Script must be a non-empty string", durationMs: Date.now() - start };
      }
      ...
      return {
        ...
        output: JSON.stringify({
          ...
          note: "JavaScript evaluation requires Playwright runtime. See browser_click for install instructions.",
        }),
        durationMs: Date.now() - start,
      };
    } catch (error) {
